...
class MovieAdapter(private val movieList: List<MovieModel>, private val context: Context):
    ...
    override fun onBindViewHolder(holder: MovieAdapter.ViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            context.startActivity(Intent(context, DetailsActivity::class.java).putExtra("name"
                ,movieList[position].name).putExtra("rating",movieList[position].rating)
                .putExtra("image",movieList[position].image))
